/**
 * Pure command parsing/resolution. No I/O — takes a line of input plus a
 * snapshot of what `look` last showed (so short indices like `b2` or `3`
 * resolve to real IDs), and returns a typed Action for the caller to
 * execute. Fully unit-testable in isolation.
 *
 * This is a deliberate copy of packages/cli/src/commands.ts, not a shared
 * import: cli is an app (no stable package boundary to depend on), and this
 * file is small, pure, and has its own test coverage here. If you fix a bug
 * in one copy, check the other.
 */
